...
function checkOriginalPageAbnormal(htmlParser) {
  const oldThead = ['EAN/UPC', 'Desp', 'Cat.', 'Group', 'Code', 'Remark1', 'Price', 'Qty', 'GIT', 'P00', 'P03', 'P05', 'P06', 'PHQ', 'PPJ', 'WS1', 'WXR'];
  ...
  for (let n = 0; n < oldThead.length; n++) {
    ...
    let fetchInnerText = htmlParser.querySelector(`thead tr th:nth-child(${n + 1})`).innerHTML;
...
async function fetchData(inputdata) {
  ...
  try {
    ...
    const response = await fetch(`http://stockbal.phcocap.com/default4.aspx?s=1&c=${inputdata}&q=${x}`);
    ...
    if (allData.ean.length > 500 && allData.ean.length < 1000) {
      playAlertSound();
      // alertSound.play();
      alert(`Warning! ${allData.ean.length} item been found, maybe will cause the system to slow down or crash.`);
    } else if (allData.ean.length >= 1000) {
      playAlertSound();
      // alertSound.play();

      let r = confirm(`Warning! ${allData.ean.length} item  been found, maybe will cause the system to crash. if you want continue click "ok"`);
...
function clickHeadAddDecorationUnderline() {
  const headerId = ['price', 'qty', 'git', 'p00', 'p03', 'p05', 'p06', 'phq', 'ppj', 'wxr'];
  ...
  headerId.forEach(id => {
    document.getElementById(id).addEventListener('click', () => {
      ...
      if (id !== 'price') {
        ...
        headerId.filter(el => el !== id).forEach(ID => {
...
function whenHeaderBeenClickChangeListColor(id) {
  const listId = ['list-qty', 'list-git', 'list-p00', 'list-p03', 'list-p05', 'list-p06', 'list-phq', 'list-ppj', 'list-wxr'];
...
function toPriceTagPage() {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', priceTagContainer);
  priceTagCancel();
  priceTagAddButton();
...
function clickPriceTag() {
  ...
  priceTagButton.addEventListener('click', () => {
    ...
    priceTagButton.setAttribute('class', 'btn btn-default btn-search btn-danger');
...
function clickQuotation() {
  ...
  quotation.addEventListener('click', () => {
    ...
    quotation.setAttribute('class', 'btn btn-default btn-search btn-danger quotation-to-back quotation-back');
...
function deleteButtonDisplayNone() {
  const deleteTheadQuotation = document.getElementById('delete-thead-quotation');
  ...
  allEan.forEach(tag => {
    ...
    if (takeOutAllData.ean.length === 1) {
      ...
      setTimeout(() => {
        ...
        document.getElementById('search-button').setAttribute('class', 'btn btn-default btn-search btn-warning')
        document.getElementById('search-button').innerHTML = 'Auto Add';

        setTimeout(() => {
          document.getElementById('search-button').setAttribute('class', 'btn btn-default btn-search btn-success')
...
function renderDataToPriceTagPage() {
  ...
  quotationEan.forEach((obj, index) => {
    ...
    <div class="price-tag-unit">
      ...
      <div>
      <span>
        <svg id="barcode-${obj.ean}-${index}" class="barcode" style="display: inline-block;"></svg>
      </span>
      <span>
    ...
    document.getElementById('price-tag-area').insertAdjacentHTML('beforeend', priceTag);
...
function renderFirebaseToQuotationTable() {
  ...
  //   document.getElementById('quotation-tbody').insertAdjacentHTML('beforeend', quotationTbody);
  // });
...
function calculateQuotation() {
  ...
  document.getElementById('total-amount').insertAdjacentHTML('afterbegin', totalAmount);
...
function deletePriceTag() {
  for (let i = 0; i < quotationEan.length; i++) {
    document.getElementById(`price-tag-delete-${i}`).addEventListener('click', () => {
      playClickSound();
...
function deleteItemQuotation() {
  ...
  for (let i = 0; i < quotationEan.length; i++) {
    document.getElementById(`quotation-delete-${i}`).addEventListener('click', () => {
...
function replaceErrorImage(num) {
  ...
  const errorLink = 'https://icon-library.com/images/not-found-icon/not-found-icon-24.jpg';
...
function fetchFirebaseToHistoryList() {
  ...
  chrome.runtime.sendMessage({ command: 'fetch' }, response => {
    ...
    response.data.forEach(item => {
      tbodyHistory.insertAdjacentHTML('afterbegin', trHistory(item.id, item.quotationData.data.fileName, item.quotationData.data.date))
    })
  })
...
function catchFirebaseIdData() {
  ...
  chrome.runtime.sendMessage({ command: 'id-data' }, response => {
    ...
    for (let i = 0; i < quotationData.length; i++) {
      let quotationData = response.data.dataItem.quotationEan;
      let price = parseFloat(quotationData[i].price.replace(/\,/g, '') * quotationData[i].value);
...
<div id="quotation-button-container" class="container-total">
  <h3 id="total-amount-title">TOTAL AMOUNT</h3>
  <h3 id="total-amount-rm">RM<span id="total-amount">${AMOUNT.toFixed(2)}</span></h3>
</div>
...
function renderFirebaseDataToQuotationTable(objData) {
  ...
  objData.forEach((obj, index) => {
    ...
    <td style='text-align:center; vertical-align:middle' id="unit-price-${index}">${obj.price}</td>
    <td style='text-align:center; vertical-align:middle' id="price-${index}">${amount.toFixed(2)}</td>
...
async function fetchDataPelw() {
  ...
  try {
    ...
    const response = await fetch(`http://stockbal.phcocap.com/default4.aspx?s=1&c=pelw&q=0`);
    const html = await response.text();
...
function pelwPriceToArr(tbody) {
  ...
  // tbody.querySelectorAll('tbody tr td:nth-child(2)').forEach(tag => obj.name.push(tag.innerHTML));
  tbody.querySelectorAll('tbody tr td:nth-child(7)').forEach((tag, i) => arrPrice.push(tag.innerHTML));
  // tbody.querySelectorAll('tbody tr td:nth-child(8)').forEach(tag => obj.qty.push(tag.innerHTML));
  // tbody.querySelectorAll('tbody tr td:nth-child(9)').forEach(tag => obj.git.push(tag.innerHTML));
  // tbody.querySelectorAll('tbody tr td:nth-child(10)').forEach(tag => obj.p00.push(tag.innerHTML));
  // tbody.querySelectorAll('tbody tr td:nth-child(11)').forEach(tag => obj.p03.push(tag.innerHTML));
...
function findPelwLocation() {
  ...
  allName.forEach((tag, i) => {
    ...
    if (findP === true && find5years === false) {
      if (PELW3003) {
        result.push({ location: i, pelw: 'PELW3003', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW4003) {
        result.push({ location: i, pelw: 'PELW4003', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW2001) {
        result.push({ location: i, pelw: 'PELW2001', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW3002) {
        result.push({ location: i, pelw: 'PELW3002', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW6003) {
        result.push({ location: i, pelw: 'PELW6003', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW4002) {
        result.push({ location: i, pelw: 'PELW4002', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW3001) {
        result.push({ location: i, pelw: 'PELW3001', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW8003) {
        result.push({ location: i, pelw: 'PELW8003', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW4001) {
        result.push({ location: i, pelw: 'PELW4001', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW6002) {
        result.push({ location: i, pelw: 'PELW6002', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW8002) {
        result.push({ location: i, pelw: 'PELW8002', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW8001) {
        result.push({ location: i, pelw: 'PELW8001', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
      if (PELW2002) {
        result.push({ location: i, pelw: 'PELW2002', price: Number(allPrice[i].innerText.replace(/,/g, '')) });
        return;
      }
...
function highLightText(arr) {
  ...
  arr.forEach(obj => {
    obj.text = obj.text.replace('BUNDLE', '<span style="color: #f44336;">BUNDLE</span>');
...
function updateHighlight(arr) {
  ...
  arr.forEach(obj => {
    ...
    // ${pelwPrice}<div style='font-size: 1rem;color: red;'>5 years warranty *</div>`;
    // document.querySelectorAll('#list-price')[obj.location].classList.add('pelw-color')
    document.querySelectorAll('#list-ean-name')[obj.location].innerHTML = obj.text;
  })
...
function highLightTextSM(arr) {
  ...
  arr.forEach(obj => {
    obj.text = obj.text.replace('(SM)', '<span style="color: #f44336;">(SM)</span>');
    afterHighlight.push(obj);
  })
...
function highLightTextDS(arr) {
  ...
  arr.forEach(obj => {
    obj.text = obj.text.replace('(DS)', '<span style="color: #f44336;">(DS)</span>');
    afterHighlight.push(obj);
  })
...
function highLightText5Y(arr) {
  ...
  arr.forEach(obj => {
    obj.text = obj.text.replace('5YEARS', '<span style="color: #f44336;">5YEARS</span>');
    afterHighlight.push(obj);
  })
